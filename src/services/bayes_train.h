...
public:
  Bayes_Train() { serverInstance = NULL; }

  Bayes_Train(GNet::GServer *newInstance) { serverInstance = newInstance; }
  ...
  shmea::ServiceData *execute(const shmea::ServiceData *data) {
    ...
    shmea::GTable inputTable("datasets/btest.csv", ',',
                             shmea::GTable::TYPE_FILE);
  ...
  shmea::GString getName() const { return "Bayes_Train"; }
};
