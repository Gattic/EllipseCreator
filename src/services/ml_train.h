...
public:
  ML_Train() { serverInstance = NULL; }

  ML_Train(GNet::GServer *newInstance) { serverInstance = newInstance; }
  ...
  shmea::ServiceData *execute(const shmea::ServiceData *data) {
    ...
    // int64_t trainPct = cList.getLong(4), testPct = cList.getLong(5),
    // validationPct = cList.getLong(6);
  ...
  shmea::GString getName() const { return "ML_Train"; }
};
